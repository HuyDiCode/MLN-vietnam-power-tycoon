...
export default function KpiDashboard({ kpis }: KpiDashboardProps) {
  const getStatus = (value: number, danger: number, warning: number) => {
    ...
    if (value <= warning) return { status: "CẢNH BÁO", color: "text-orange-400" }
    ...
    <Card className="bg-slate-800 border-2 border-slate-700 p-6 space-y-4">
      ...
      <div className="space-y-4">
        {Object.entries(kpis).map(([key, value]) => {
          ...
          const { status, color } = getStatus(value, config.danger, config.warning)
          const progressColor = getProgressColor(value, config.danger, config.warning)
            ...
            <div key={key} className="bg-slate-700 p-4 rounded border border-slate-600">
              <div className="flex justify-between items-center mb-2">
                ...
                <div className="text-right">
                  <div className={`font-bold ${config.color}`}>{Math.round(value)}</div>
