      ...
      <Card className="max-w-2xl w-full bg-slate-800 border-2 border-slate-700">
        <div className="p-8 space-y-6">
          ...
          <div className="text-center">
            <h1 className="text-4xl font-bold text-blue-400 mb-2">{getEndingTitle()}</h1>
          ...
          {result.finalKpis && (
            <div className="bg-slate-700 p-6 rounded border border-slate-600">
              <h3 className="text-lg font-bold text-blue-400 mb-4">Chỉ Số Cuối Cùng:</h3>
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-400">{Math.round(result.finalKpis.finance)}</div>
                  <div className="text-sm text-gray-400">Tài chính</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-yellow-400">{Math.round(result.finalKpis.energy)}</div>
                  <div className="text-sm text-gray-400">Năng lượng</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-red-400">{Math.round(result.finalKpis.satisfaction)}</div>
                  <div className="text-sm text-gray-400">Hài lòng</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-purple-400">{Math.round(result.finalKpis.politics)}</div>
